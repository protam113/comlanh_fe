...
import ComLanhSchoolCard from '@/components/card/intro.section';
import { SchoolHospitalFactoryCard } from '@/components/card/doi-tac/thbvxn';
import SchoolHospitalFactoryCategories from '@/components/card/doi-tac/thbvxn-cte';
...
export default function BlogList() {
  ...
  return (
    <div className="pt-16 bg-white min-h-screen">
      {/* Breadcrumbs */}
      <PublicContainer className="bg-section-alt mt-4 mx-auto py border-b  border-gray-200">
        <div className="container-custom">
          <nav className="flex items-center gap-2 text-[0.85rem] font-bold text-text-muted uppercase tracking-wider">
            ...
            <span className="text-primary-dark">
              Trường Học - Bênh Viện - Xí Nghiệp
            </span>
          </nav>
        </div>
      </PublicContainer>
      <PublicContainer className="mx-auto py-8 space-y-6">
        {/* <ComLanhSchoolCard /> */}
        <SchoolHospitalFactoryCard />
        <CTAButton />
        <SchoolHospitalFactoryCategories />

        <h1 className=" text-3xl md:text-4xl font-black text-main mb-12 flex items-center gap-4">
          Tất cả bài viết
            ...
            >
              ...
              <div className="flex flex-col justify-center py-2">
                ...
                <h2 className=" text-2xl md:text-3xl font-black text-text-secondary mb-4 group-hover:text-primary transition-colors duration-300 leading-tight">
                  <Link href={`/tin-tuc/${article.id}`}>{article.title}</Link>
                </h2>
