      ...
      <div className="container-custom">
        <div className="flex items-center justify-between py-2 gap-8">
          ...
          >
            {navItems.map((item) => (
              <div key={item.label} className="relative group/item">
                <Link
                  href={item.href}
                  className={`flex items-center gap-1 px-3.5 py-2 text-[0.9rem] font-medium rounded-sm transition-all ${
                    item.active
                      ? 'text-primary'
          ...
          <div className="flex items-center gap-4 shrink-0">
            <Link
              href="/lien-he"
              className="bg-primary text-white px-6 py-2 rounded-full text-[0.85rem] font-black shadow-warm-sm hover:bg-primary-dark hover:-translate-y-0.5 transition-all"
            >
              LIÊN HỆ
            </Link>
